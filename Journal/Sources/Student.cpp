...
Student::Student(int newId, std::string newName, std::string newSurname, std::string newPatronymic, int newAge, std::string newSex, std::string newAddress)
    :Person(newId, newName, newSurname, newPatronymic, newAge, newSex, newAddress) {
            if(showConDesMessages) std::cout<< rang::fg::cyan << "Constructor of Student class was called!" << rang::fg::reset << std::endl;
        }

Student::~Student() {
    if(showConDesMessages) std::cout << rang::fg::blue << "Destructor of Student class was called!" << rang::fg::reset << std::endl;
}
